...
const GENDERS_GAY = GENDER_IDENTITY_OPTIONS;
const SEXUAL_PREFERENCES_GAY = SEXUAL_PREFERENCE_OPTIONS.map((label) => ({ label, value: label.toLowerCase() }));
const LOOKING_FOR_GAY = LOOKING_FOR_OPTIONS.map((label) => ({
  label,
  value:
    label === "Men"
      ? "men"
...
function GayChip({ label, selected, onClick }) {
  return (
    <button
      onClick={onClick}
...
const COMPAT_SECTIONS = [
  {
    ...
    questions: [
      { id: "overall_attraction_direction", title: "Overall Attraction Direction", type: "slider", prompt: "What overall look are you usually most drawn to?", minLabel: "Soft / boyish", maxLabel: "Sharp / defined" },
      { id: "face_aesthetic_pref", title: "Face & Presence Type", type: "multi_select", prompt: "Which overall looks tend to feel like your type?", microcopy: "Select all that feel honest.", opts: ["cute_boy_next_door","handsome_classic","rugged_masculine","pretty_refined","striking_editorial","mature_sophisticated","soft_gentle","intense_confident","distinctive_presence","not_sure_yet"] },
      { id: "grooming_style_pref", title: "Grooming & Self-Care", type: "multi_select", prompt: "Which presentation styles do you find attractive?", opts: ["clean_put_together","slightly_scruffy","very_well_groomed","natural_low_effort","intentional_style","healthy_fresh","polished_confident","looks_after_themselves","expressive_presentation","no_strong_preference"] },
      { id: "facial_hair_pref", title: "Facial Hair Preferences", type: "multi_select", prompt: "Which facial hair styles are you usually into?", opts: ["clean_shaven","light_stubble","heavy_stubble","short_beard","full_beard","moustache","goatee","body_hair_and_beard_combo","no_strong_preference"] },
      { id: "hair_style_pref", title: "Hair & Head Look", type: "multi_select", prompt: "Which hair styles or head looks are you usually drawn to?", opts: ["bald_shaved","buzz_cut","short_clean","textured_short","medium_flow","long_hair","curly_textured","dyed_alternative","polished_style","no_strong_preference"] },
      { id: "face_detail_pref", title: "Facial Details", type: "multi_select", maxSelect: 5, prompt: "What facial details tend to stand out most to you?", microcopy: "Select up to 5.", opts: ["nice_smile","straight_teeth","bright_eyes","expressive_eyes","strong_jawline","soft_features","intense_look","warm_friendly_expression","distinctive_face","natural_ease"] },
      { id: "body_type_pref", title: "Body & Physical Build", type: "multi_select", prompt: "Which body types do you usually find attractive?", opts: ["slim","lean","athletic","toned","muscular","broad","stocky","soft_dadbod","no_strong_preference"] },
      { id: "style_visual_pref", title: "Style & Visual Signals", type: "multi_select", maxSelect: 6, prompt: "Which style signals make someone more your type?", microcopy: "Pick your top 6.", opts: ["clean_cut","sporty","casual","rugged","artsy","streetwear","minimal","fashion_forward","classic_elegant","glasses","tattoos","piercings","jewellery","body_hair","gym_or_wellness_led","scene_polished","low_key_natural"] },
  ...
  },
  {
    ...
    questions: [
      { id: "past_dating_types", title: "Past Dating Patterns", type: "multi_select", prompt: "Which kinds of guys have you dated or been drawn to before?", opts: ["emotionally_available","emotionally_unavailable","fun_adventurous","serious_stable","confident_assertive","soft_sensitive","ambitious_driven","creative_free_spirited","nurturing_caring","independent_distant","high_chemistry_low_stability","calm_grounded","expressive_intense","low_key_private","socially_outgoing","scene_connected","avoidant_guarded","affectionate_warm","inconsistent_mixed_signals","relationship_ready","still_figuring_things_out"] },
  ...
  },
  {
    ...
    questions: [
      { id: "lifestyle_identity", title: "Lifestyle Identity", type: "multi_select", prompt: "Which lifestyle patterns feel most like you?", opts: ["social_and_outgoing","low_key_and_private","routine_oriented","spontaneous","scene_connected","non_scene","saver","spender","career_first","balance_first","planner","flexible","wellness_focused","nightlife_oriented","culture_or_creativity_led"] },
  ...
  },
  {
    ...
    questions: [
      ...
      { id: "sexual_role", title: "Sexual Role", type: "single_select", prompt: "What feels most natural for you sexually?", microcopy: "Private and only used to improve compatibility.", opts: ["top","bottom","versatile","side","flexible_depends_on_chemistry","still_exploring"] },
      { id: "sexual_dynamic_preference", title: "Sexual Dynamic Preference", type: "single_select", prompt: "What dynamic usually works best for you?", microcopy: "Private and only used to improve compatibility.", opts: ["clearly_complementary_dynamic","another_versatile_person","flexible_depends_on_connection","emotionally_led_chemistry","low_pressure_intimacy_first","still_exploring"] },
...
const getStages = () => [
  "welcome", "basics", "metrics", "photos", "prompts", "verification",
...
// Sizing system for onboarding banners
const ONBOARDING_LAYOUT = {
  mainBannerWidth: "100%",           // Get started button
  mainBannerMaxWidth: "360px",       // Constrain to readable width
  mainBannerHeight: "48px",          // Pill button height
  infoBannerWidth: "100%",           // What makes our app different (same as Get started)
  infoBannerMaxWidth: "360px",       // Match Get started exactly
  membershipBannerMaxWidth: "220px", // Membership banner width - compact but not squeezed
  membershipBannerHeight: "auto",    // Allow flexible height for 2-line text
};

// Lower banner system colours — membership selector only
        ...
        {open && (
          ...
          >
            <div className="p-4 space-y-4">
              {/* Block 1 — Why we're different */}
              <div>
                <p className="text-[11px] font-semibold uppercase tracking-widest mb-2" style={{ color: mutedColor }}>
                  Why we're different
                </p>
                <p className="text-xs leading-relaxed mb-2" style={{ color: textColor }}>
                  Tether is built around trust, compatibility, and real-world dating — not endless swiping.
                </p>
                <p className="text-xs leading-relaxed mb-3" style={{ color: subTextColor }}>
                  Most dating apps focus on surface-level filters, fast matching, and keeping people engaged inside the app. We take a more intentional approach.
                </p>
              ...
              {/* Block 2 — Compatibility threshold */}
              <div>
                <p className="text-sm font-heading font-semibold mb-2" style={{ color: headingColor }}>
              ...
              <div className="border-t" style={{ borderColor }} />
              {/* Block 3 — Compatibility test */}
              <div>
                <p className="text-sm font-heading font-semibold mb-2" style={{ color: headingColor }}>
                  Our compatibility test makes us different
                </p>
                <p className="text-xs leading-relaxed mb-2" style={{ color: textColor }}>
                  Our compatibility system is designed to go beyond age, location, and surface preferences.
                </p>
                <p className="text-xs leading-relaxed mb-3" style={{ color: subTextColor }}>
                  Instead of relying on shallow matching, we use a more thoughtful framework shaped by psychological insight, dating expertise, and real-world behaviour over time.
                </p>
                <p className="text-xs leading-relaxed mb-3" style={{ color: subTextColor }}>
                  Your assessment helps us understand your type, dating patterns, values, lifestyle fit, and the kind of chemistry that tends to work best for you.
                </p>
                <p className="text-xs leading-relaxed mb-3" style={{ color: subTextColor }}>
                  We do not believe dating should stop at the match. Our system is built to keep learning from compatibility signals, behaviour, and outcomes so it can continue improving over time.
                </p>
                <p className="text-xs font-semibold mb-3" style={{ color: tier === "standard" ? "#2F3B35" : "rgba(238,231,218,0.9)" }}>
...
function formatSliderAxisLabel(axis, question) {
  return formatAssessmentDisplayLabel(question?.axisLabels?.[axis] || axis);
}

function formatOptionLabel(option, question) {
  return formatAssessmentDisplayLabel(question?.optionLabels?.[option] || option);
}
    ...
    <div className="assessment-question-shell flex-1 flex flex-col" data-testid="assessment-question-screen">
      <div className="assessment-question-content" ref={scrollRef}>
        <div className="assessment-question-panel" data-testid="assessment-question-panel">
          <p className="assessment-microcopy mb-3 text-center">Question {questionIndex + 1} of {section.questions.length}</p>
          <p className="assessment-question-title mb-2 text-center" data-testid="assessment-question-title">{question.title || `Question ${questionIndex + 1}`}</p>
          ...
          <p className="assessment-question-prompt assessment-question-prompt-heading mb-2 text-center mx-auto max-w-[28ch]" style={{ textAlign: "center" }} data-testid="assessment-question-prompt">{question.prompt}</p>
          <p className="assessment-question-microcopy mb-3 text-center text-[12.5px]" style={{ textAlign: "center" }} data-testid="assessment-question-microcopy">{getQuestionMicrocopy(question)}</p>

          {(question.type === "single_select" || !question.type) && (
            <div className={`assessment-option-list assessment-option-list--gay-preview ${question.opts.length >= 7 ? "assessment-option-list--dense" : ""} max-w-[86%] gap-3`} data-testid="assessment-option-list">
              {question.opts.map((opt) => (
                <button
                  key={opt}
                  onClick={() => {
                    setValidationMessage("");
                    setAnswers(prev => ({ ...prev, [questionIndex]: opt }));
                  }}
                  className="assessment-option-card assessment-option-card--gay-preview w-full text-center justify-center min-h-[36px] py-1.5 px-4"
                  data-testid="assessment-option-card"
                  data-selected={answers[questionIndex] === opt ? "true" : "false"}
                  type="button"
                >
                  <span className="assessment-option-title text-[0.86rem] leading-tight text-center">{formatOptionLabel(opt, question)}</span>
                </button>
              ))}
            </div>
          )}
              ...
              {question.opts.map((opt) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      setValidationMessage("");
                      toggleMulti(questionIndex, opt, question.maxSelect);
                    }}
                    className="assessment-option-card assessment-option-card--gay-preview w-full text-center justify-center min-h-[36px] py-1.5 px-4"
                    data-testid="assessment-option-card"
                    data-selected={selected ? "true" : "false"}
          ...
          {question.type === "slider" && (
            <div className="assessment-option-card">
              <Slider
                data-testid="assessment-slider"
                value={answers[questionIndex] || [0.5]}
                onValueChange={(val) => {
                  setValidationMessage("");
          ...
          {question.type === "slider_set" && (
            <div className="space-y-2">
              <p className="assessment-microcopy text-center">Scale: Not like me · Somewhat · Very true</p>
              {(question.axes || []).map((axis) => (
                <div key={axis} className="assessment-option-card assessment-option-card--slider-axis">
                  <p className="assessment-axis-title mb-2 text-center">{formatSliderAxisLabel(axis, question)}</p>
                  <Slider
                    data-testid="assessment-slider"
                    value={[answers[questionIndex]?.[axis] ?? 0.5]}
                    onValueChange={([v]) => {
                      setValidationMessage("");
                      setAnswers(prev => ({ ...prev, [questionIndex]: { ...(prev[questionIndex] || {}), [axis]: v } }));
                    }}
                    min={0}
                    max={1}
                    step={0.01}
                  />
                  <div className="assessment-slider-label-row">
          ...
          {validationMessage ? <p className="assessment-validation-message mt-3 text-center w-full" data-testid="assessment-validation-message">{validationMessage}</p> : null}
        </div>
      </div>
...
export default function Onboarding() {
  ...
  const [compatSectionPhase, setCompatSectionPhase] = useState("intro");
  ...
  const resolvedCompatibilityRoute = determineCompatibilityVariant({
    ...
    sexualPreference: selectedPref || "gay",
    lookingFor,
  });
  ...
  useEffect(() => {
    if (!gender) return;
    setFormData((prev) => applyDefaultProfileDetails(prev, gender, selectedPref, lookingFor));
  ...
  const handleCompatSectionComplete = (data) => {
    const sectionId = compatSections[compatSectionIndex].id;
    setCompatMarkers(prev => ({ ...prev, [sectionId]: data }));
    setCompatQuestionIndex(0);
    if (compatSectionIndex < compatSections.length - 1) {
      setCompatSectionIndex(i => i + 1);
      setCompatSectionPhase("intro");
    } else {
  ...
  const handleBasicsContinue = () => {
    ...
    const age = calcAge();
    const effectivePref = normalizeSexualPreference(selectedPref || "gay");
  ...
  const handleComplete = async () => {
    setLoading(true);
    const computedAge = calcAge() || Number(formData.age);
    ...
    const profile = {
      ...
      sexual_preference: normalizeSexualPreference(selectedPref || "gay"),
      sexual_preference_label: SEXUAL_PREFERENCES_GAY.find((item) => item.value === (selectedPref || "gay"))?.label || selectedPref || "Gay",
      dating_preference: normalizeLookingFor(lookingFor),
      dating_preference_label: LOOKING_FOR_GAY.find((item) => item.value === lookingFor)?.label || lookingFor,
  ...
  const goBack = () => {
    ...
    if (stage === "compat_section" && compatSectionPhase === "questions") {
      window.dispatchEvent(new Event("tether-compat-question-back"));
      return;
    }
    if (stage === "compat_section" && compatSectionPhase === "intro" && compatSectionIndex > 0) {
      setCompatSectionIndex((prev) => prev - 1);
      setCompatSectionPhase("intro");
      setCompatQuestionIndex(0);
      scrollToTopSmooth();
      return;
  ...
  const canProceed = () => {
    if (stage === "basics") return formData.display_name && calcAge() && gender && selectedPref && lookingFor && formData.location === "London";
    return true;
  };

  const currentSection = compatSections[compatSectionIndex];
  useEffect(() => {
    ...
    if (typeof draft.compatSectionIndex === "number") setCompatSectionIndex(draft.compatSectionIndex);
    if (draft.compatSectionPhase) setCompatSectionPhase(draft.compatSectionPhase);
  ...
  }, [compatQuestionIndex, compatSectionIndex, compatSectionPhase, dob, formData, gender, location.pathname, lookingFor, navigate, resolvedCompatibilityRoute.onboardingRoute, selectedPref, stage]);
  ...
  useEffect(() => {
    ...
    const sectionMatch = path.match(/\/onboarding(?:-gay)?-section(0[1-5]|[1-5])$/);
    if (sectionMatch) {
      const nextSectionIndex = Math.max(0, Math.min(4, Number(sectionMatch[1]) - 1));
      setCompatSectionIndex(nextSectionIndex);
      setCompatSectionPhase("intro");
      setCompatQuestionIndex(0);
    ...
    if (path.endsWith("/onboarding-howitworks") || path.endsWith("/onboarding-gay-howitworks")) {
      setHowTetherWorksPageIndex(0);
      setStage("date_journey");
    }
  }, [location.pathname]);
        ...
        <div className="flex gap-1" data-testid="assessment-progress">
            {Array.from({ length: progressTotal }).map((_, i) => (
              <div
                ...
                style={{
                  backgroundColor: i < progressActive ? tierColors.bg : "hsl(var(--border))",
                  opacity: i < progressActive ? 1 : 0.22,
                }}
              />
            ))}
          </div>
      ...
      <div className="flex-1 flex flex-col px-5 py-2 max-w-2xl mx-auto w-full">
        {stage !== "welcome" && (
          <div className="onboarding-top-row mb-3">
            <button
              onClick={goBack}
              className="onboarding-back-button flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors"
        ...
        <AnimatePresence mode="wait">
          <motion.div
            key={stage + (stage === "compat_section" ? compatSectionIndex + compatSectionPhase : "")}
          ...
          >
              ...
              <div className="flex-1 flex flex-col gap-5 overflow-auto pb-2" data-testid="about-you-screen">
                ...
                <div className="space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <OnboardingSelectField label="Pronouns" value={formData.pronouns} onChange={(value) => update("pronouns", value)} options={PRONOUN_OPTIONS} placeholder="Select pronouns" />
                  </div>
                  ...
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField label="Ethnicity" value={formData.ethnicity} onChange={(value) => update("ethnicity", value)} options={ETHNICITY_OPTIONS} placeholder="Select ethnicity" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                    <OnboardingSelectField
                      ...
                      options={GENDERS_GAY.map((label) => ({ label, value: label }))}
                      placeholder="Select"
                      className="flex h-full flex-col"
                      triggerClassName="h-full min-h-9"
                    />
                  ...
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField
                      ...
                      options={SEXUAL_PREFERENCES_GAY}
                      placeholder="Select"
                      className="flex h-full flex-col"
                      triggerClassName="h-full min-h-9"
                    />
                  ...
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    ...
                    <OnboardingSelectField label="Children" value={formData.children} onChange={(value) => update("children", value)} options={CHILDREN_OPTIONS} placeholder="Select children preference" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                  </div>
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField label="Future family desire" value={formData.future_family_desire} onChange={(value) => update("future_family_desire", value)} options={FUTURE_FAMILY_DESIRE_OPTIONS} placeholder="Select future family desire" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                    <OnboardingSelectField label="Pets" value={formData.pets} onChange={(value) => update("pets", value)} options={PET_OPTIONS} placeholder="Select pets" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                  </div>
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField label="Drinking" value={formData.drinking} onChange={(value) => update("drinking", value)} options={DRINKING_OPTIONS} placeholder="Select drinking" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                    <OnboardingSelectField label="Smoking" value={formData.smoking} onChange={(value) => update("smoking", value)} options={SMOKING_OPTIONS} placeholder="Select smoking" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                  </div>
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField label="Drugs" value={formData.drugs} onChange={(value) => update("drugs", value)} options={DRUGS_OPTIONS} placeholder="Select drugs" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                    <OnboardingSelectField label="Religion" value={formData.religion} onChange={(value) => update("religion", value)} options={RELIGION_OPTIONS} placeholder="Select religion" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                  </div>
                  <div className="grid grid-cols-2 items-stretch gap-3">
                    <OnboardingSelectField label="Politics" value={formData.politics} onChange={(value) => update("politics", value)} options={POLITICS_OPTIONS} placeholder="Select politics" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                    <OnboardingSelectField label="Languages" value={formData.languages} onChange={(value) => update("languages", value)} options={LANGUAGE_OPTIONS} placeholder="Select languages" className="flex h-full flex-col" triggerClassName="h-full min-h-9" />
                  </div>
            ...
            {stage === "metrics" && (
              <div className="flex-1 flex flex-col gap-6">
                ...
                <div className="space-y-5">
                  <div>
                    <label className="app-section-title text-muted-foreground mb-3 block">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mb-2">We may occasionally flex this for exceptional matches.</p>
            ...
            {stage === "photos" && (
              <div className="flex-1 flex flex-col gap-5">
                ...
                <div className="grid grid-cols-3 gap-2.5">
                  {[0, 1, 2, 3, 4].map(i => (
                    <label key={i} className={`${i === 0 ? "col-span-2 row-span-2" : ""} aspect-[3/4] rounded-2xl border-2 border-dashed flex items-center justify-center cursor-pointer transition-all hover:border-primary/40 overflow-hidden ${formData.photos[i] ? "border-transparent" : "border-border"}`}>
                      {formData.photos[i] ? (
                        <img src={formData.photos[i]} className="w-full h-full object-cover rounded-2xl" alt="" />
                      ) : (
            ...
            {stage === "prompts" && (
              <div className="flex-1 flex flex-col gap-5">
                ...
                <div className="space-y-4">
                  ...
                  <OnboardingMultiSelectField label="Relationship Rhythm" values={formData.relationship_rhythm} onChange={(value) => update("relationship_rhythm", value)} options={RELATIONSHIP_RHYTHM_OPTIONS} placeholder="Select relationship rhythm" />
                  <OnboardingMultiSelectField label="Small things I value" values={formData.small_things_i_value} onChange={(value) => update("small_things_i_value", value)} options={SMALL_THINGS_I_VALUE_OPTIONS} placeholder="Select what you value" />
                  <OnboardingMultiSelectField label="Green Flags" values={formData.green_flags} onChange={(value) => update("green_flags", value)} options={GREEN_FLAGS_OPTIONS} placeholder="Select green flags" />
                </div>
              </div>
            )}
                ...
                <div className="verification-centred-area">
                  <div className="verification-stack" data-testid="verification-stack">
                    ...
                    <div>
                      ...
                      <p className="app-lead text-muted-foreground max-w-xs leading-relaxed">
                        Every Tether member is verified to create a safer, more trustworthy dating experience.
                      </p>
                    </div>
            ...
            {stage === "compat_intro" && (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-6 assessment-screen--centered assessment-intro-screen" data-testid="assessment-intro-screen">
                ...
                <div className="assessment-intro-copy">
                  <p className="text-xs font-semibold uppercase tracking-widest mb-3 assessment-intro-kicker" style={{ color: tierColors.bg }}>Compatibility Assessment</p>
                  ...
                  <p className="assessment-body leading-relaxed max-w-sm">
                    Next, you'll complete a five-part assessment. This isn't a questionnaire — it's the system learning who you are, what you want, and who you're likely to connect with.
                  </p>
                </div>
                ...
                {compatSectionPhase === "intro" ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center gap-7 assessment-screen--centered" data-testid="assessment-section-intro-screen">
                    ...
                    <div className="max-w-xs">
                      ...
                      <blockquote className="assessment-section-quote mb-4" data-testid="assessment-section-intro-quote">
                    ...
                    <Button onClick={() => { setCompatQuestionIndex(0); setCompatSectionPhase("questions"); }} className="rounded-full w-full max-w-xs" style={{ backgroundColor: tierColors.bg, color: tierColors.text }} data-testid="assessment-section-begin">
                      Begin <ArrowRight className="w-4 h-4 ml-2" />
                    </Button>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col">
                    <div className="assessment-gay-section-header text-center">
                      <p className="assessment-kicker">SECTION {currentSection.number} / 05</p>
                      <h2 className="assessment-section-title" data-testid="assessment-section-title">{currentSection.title}</h2>
                    </div>
                    <CompatSection
                      section={currentSection}
                      onComplete={handleCompatSectionComplete}
                      onBackToIntro={() => setCompatSectionPhase("intro")}
                      onQuestionIndexChange={setCompatQuestionIndex}
                    />
                  </div>
                )}
              </div>
            ...
            {stage === "complete" && (
              <div className="flex-1 flex flex-col items-center justify-center text-center gap-7">
                ...
                <div>
                  ...
                  <p className="app-lead text-muted-foreground max-w-xs leading-relaxed">
                    Your compatibility profile is ready. We'll curate your first set of daily matches based on everything you've shared.
                  </p>
                </div>
                <Button onClick={handleComplete} size="lg" className="w-full max-w-xs rounded-full" disabled={loading}>
                  {loading ? "Setting up your profile..." : "Choose your membership"}
        ...
        {["basics", "metrics", "photos", "prompts", "verification", "compat_intro"].includes(stage) && (
          <div className={`flex items-center gap-3 pt-4 pb-3 ${stage === "compat_intro" ? "assessment-intro-controls" : ""}`}>
            <Button variant="ghost" onClick={goBack} className="rounded-full text-sm">
              <ArrowLeft className="w-4 h-4 mr-1" /> Back
            </Button>
            {stage === "basics" ? (
              <Button onClick={handleBasicsContinue} disabled={!canProceed()} className="rounded-full flex-1 h-auto py-2.5 text-xs whitespace-normal text-center leading-snug">
                Continue to your compatibility profile <ArrowRight className="w-3.5 h-3.5 ml-1 flex-shrink-0" />
              </Button>
            ) : (
              <Button onClick={goNext} disabled={!canProceed()} className="rounded-full flex-1">
